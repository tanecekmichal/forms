...
exports.callback = function (match_field, message,fn) {
    if (!message) { message = 'Validator for %s has no error message !'; }
    var validator;
    if(typeof fn == 'string') {
        try {
            validator = JSON.parse(fn)
        } catch (e) {
            throw new Error('Error parsing function provided as validator: `'+fn+'`.');
        }
    } else if (typeof fn == 'function') {
        validator = fn;
    } else {
        throw new Error('Error: provided validator is not a function or stringified function -  `'+fn+'`.');
    }
    return (function(v){
        return function(form,field,callback) {
            if(validator(field.data)) {
